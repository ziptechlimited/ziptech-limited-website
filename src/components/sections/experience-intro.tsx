...
const ExperienceIntro: React.FC<ExperienceIntroProps> = () => {
  ...
  const p1PointerEvents = useTransform(scrollYProgress, (v) =>
    v > 0.35 ? "none" : "auto",
  );
  ...
  const p2PointerEvents = useTransform(p2EntryProgress, (v) =>
    v > 0.5 ? "auto" : "none",
  );
      ...
      <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden">
        ...
        <motion.div
          style={{
            opacity: p1Opacity,
            y: p1Y,
            pointerEvents: p1PointerEvents as any,
          }}
        ...
        >
          ...
          <div className="exp-images hidden lg:block absolute inset-0 pointer-events-none z-0">
            {images.map((img, idx) => {
              const isHovered =
                hoveredIndex !== null && hoveredIndex % images.length === idx;
          ...
          <div className="exp-content text-center max-w-[1440px] w-full z-10">
            ...
            <h2 className="exp-title text-[clamp(2.5rem,10vw,7.5rem)] leading-[0.95] tracking-[-0.04em] font-serif mb-[6vh] flex flex-col items-center justify-center cursor-default">
              <div className="w-full">
                <motion.span
                  style={{
                    filter: useTransform(p1Blur, (b) => `blur(${b}px)`),
                  }}
        ...
        <motion.div
          style={{
            opacity: p2Opacity,
            y: p2Y,
            pointerEvents: p2PointerEvents as any,
          }}
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-20">
            <svg
              ...
              className="w-[80vmin] h-[80vmin] max-w-[800px] max-h-[800px]"
            >
              {Array.from({ length: 60 }).map((_, i) => {
                ...
                return (
                  <motion.line
                    key={i}
                    x1={Number((Math.sin(angle) * innerR).toFixed(3))}
                    y1={Number((-Math.cos(angle) * innerR).toFixed(3))}
                    x2={Number((Math.sin(angle) * outerR).toFixed(3))}
                    y2={Number((-Math.cos(angle) * outerR).toFixed(3))}
                    stroke={
                      isLastTick ? "var(--accent, #f97316)" : "currentColor"
                    }
          ...
          <div className="relative z-10 text-center w-full max-w-[90vw] flex flex-col gap-[0.1em] font-medium text-[clamp(1.5rem,4vw,3.5rem)] leading-[1.15] tracking-[-0.02em]">
            ...
            {allPhase2Boxes.map((row, rowIdx) => (
              <div
                key={rowIdx}
                className="flex justify-center flex-wrap gap-[0.25em]"
              >
                {row.map((box, boxIdx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="opacity-0 select-none block">
                        {box.text}
                      </span>
